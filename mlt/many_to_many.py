...
def prepare_datasets(batch_size, dataset_size=125000):
    ...
    val_en_fr = en_fr.take(2000)
    train_en_fr = en_fr.skip(2000).take(dataset_size)
    train_fr_en = en_fr.skip(2000 + dataset_size).take(dataset_size).map(
        lambda a, b: (b, a))

    val_en_de = en_de.take(2000)
    train_en_de = en_de.skip(2000).take(dataset_size)
    train_de_en = en_de.skip(2000 + dataset_size).take(dataset_size).map(
        lambda a, b: (b, a))

    with tf.io.gfile.GFile(flags.FLAGS.encoding_model_file, "rb") as f:
        encoding_model_proto = f.read()

    val_en_fr = val_en_fr.map(lambda a, b:
    ...
    val_en_de = val_en_de.map(lambda a, b:
                              (tfs.encode(tf.expand_dims(a, 0),
                                          model_proto=encoding_model_proto,
                                          add_bos=True,
                                          add_eos=True)[0][0],
    ...
    train_data = train_en_fr.concatenate(train_fr_en).concatenate(
        train_en_de).concatenate(train_de_en)

    val_data = val_en_fr.concatenate(val_en_de)
...
def main(_):
    ...
    train_data, val_data = prepare_datasets(batch_size, dataset_size=125000)
...
if __name__ == "__main__":
    ...
    app.flags.DEFINE_integer("validation_steps", 40, "validation_steps")
